...
<template>
  <div class="flex min-h-screen flex-col items-center justify-center bg-[#F5EFE2] px-6 text-center text-[#3E2723]">
    ...
    <h1 class="mt-6 font-serif text-3xl font-bold sm:text-4xl">Ռեստորանը չգտնվեց</h1>
    <p class="mt-3 max-w-md font-serif text-[#8A7868]">
      «<span class="font-semibold text-[#A87E42]">{{ slug }}</span>» հասցեով ռեստորան գոյություն չունի։
    </p>

    <div v-if="list?.length" class="mt-8 w-full max-w-sm">
      <p class="mb-3 text-xs font-semibold uppercase tracking-widest text-[#8A7868]">Հասանելի ռեստորաններ</p>
