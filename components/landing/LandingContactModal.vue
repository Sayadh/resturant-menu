...
const submit = async () => {
  ...
  if (phone.replace(/\D/g, '').length < 6) {
    error.value = 'Խնդրում ենք մուտքագրել վավեր հեռախոսահամար'
    return
  }
  ...
  try {
    ...
    error.value = (e as Error)?.message || 'Չհաջողվեց ուղարկել, խնդրում ենք զանգահարել'
  } finally {
    ...
    <Transition name="lm">
      <div v-if="open" class="fixed inset-0 z-[100] flex items-end justify-center p-0 sm:items-center sm:p-6">
        ...
        <div class="lm-panel relative w-full max-w-md overflow-hidden rounded-t-3xl bg-white shadow-2xl sm:rounded-3xl" style="font-family:'Inter','Noto Sans Armenian',sans-serif">
          <button class="absolute right-4 top-4 grid h-9 w-9 place-items-center rounded-full text-slate-400 transition hover:bg-slate-100 hover:text-slate-700" aria-label="Փակել" @click="closeModal">
          ...
          <div v-if="done" class="px-7 py-12 text-center">
            ...
            <h3 class="mt-5 text-xl font-bold text-slate-900">Շնորհակալություն!</h3>
            <p class="mt-2 text-sm text-slate-500">Ձեր հայտը ստացվեց։ Մենք կկապվենք ձեզ հետ շատ շուտով։</p>
            <button class="mt-6 rounded-2xl bg-slate-900 px-6 py-3 text-sm font-semibold text-white transition hover:bg-slate-800" @click="closeModal">Փակել</button>
          </div>
          ...
          <div v-else class="px-7 pb-7 pt-8">
            ...
            <h3 class="mt-4 text-2xl font-extrabold tracking-tight text-slate-900">Թողեք ձեր հայտը</h3>
            <p class="mt-2 text-sm leading-relaxed text-slate-500">
              Ուղարկեք ձեր հեռախոսահամարը՝ մենք կկապվենք ձեզ հետ, կամ զանգահարեք մեզ ուղիղ։
            </p>

            <div class="mt-6 space-y-3">
              <input v-model="form.name" type="text" placeholder="Ձեր անունը (ըստ ցանկության)" class="w-full rounded-xl border border-slate-300 px-4 py-3 text-sm outline-none transition focus:border-indigo-500 focus:ring-2 focus:ring-indigo-500/20" />
              <input v-model="form.phone" type="tel" inputmode="tel" placeholder="Հեռախոսահամար *" class="w-full rounded-xl border border-slate-300 px-4 py-3 text-sm outline-none transition focus:border-indigo-500 focus:ring-2 focus:ring-indigo-500/20" @keyup.enter="submit" />
              <textarea v-model="form.message" rows="2" placeholder="Հաղորդագրություն (ըստ ցանկության)" class="w-full resize-none rounded-xl border border-slate-300 px-4 py-3 text-sm outline-none transition focus:border-indigo-500 focus:ring-2 focus:ring-indigo-500/20" />
            ...
            <div class="my-5 flex items-center gap-3 text-xs text-slate-400">
              <span class="h-px flex-1 bg-slate-200" /> կամ <span class="h-px flex-1 bg-slate-200" />
            </div>
