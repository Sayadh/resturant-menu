  ...
  <div class="landing">
    ...
    <article class="mx-auto max-w-3xl px-5 pt-32 pb-16 sm:px-8 sm:pt-40">
      ...
      <div class="mt-12 rounded-3xl bg-gradient-to-br from-indigo-600 to-violet-700 p-8 text-center text-white">
        ...
        <NuxtLink to="/#pricing" class="mt-5 inline-flex rounded-2xl bg-white px-6 py-3 text-sm font-bold text-indigo-700 shadow-lg transition hover:-translate-y-0.5">{{ L.blog.ctaBtn }}</NuxtLink>
      </div>
      ...
      <div v-if="related.length" class="mt-14">
        ...
        <div class="mt-5 grid gap-4 sm:grid-cols-3">
          <NuxtLink v-for="r in related" :key="r.slug" :to="`/blog/${r.slug}`" class="rounded-2xl border border-slate-100 bg-white p-4 text-sm font-semibold text-slate-800 shadow-sm transition hover:-translate-y-1 hover:shadow-md">
            {{ r.title }}
          </NuxtLink>
        </div>
      </div>
