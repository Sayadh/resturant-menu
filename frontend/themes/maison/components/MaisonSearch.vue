    ...
    <Transition name="ms-fade">
      <div v-if="open" class="fixed inset-0 z-50" @click="emit('close')">
        <Transition name="ms-modal" appear>
          <div
            v-if="open"
            class="maison-theme absolute inset-x-0 top-0 border-b border-[#E7DDCB] bg-[#F4EEE2] px-5 py-8 sm:px-8 sm:py-12"
            @click.stop
          >
            <div class="mx-auto flex max-w-3xl items-center gap-4">
              <svg viewBox="0 0 24 24" class="h-6 w-6 shrink-0 text-[#C4693F]" fill="none" stroke="currentColor" stroke-width="1.5" aria-hidden="true">
              ...
              <button
                type="button"
                class="shrink-0 text-[#8A7C6B] transition-colors hover:text-[#4A3B2E]"
