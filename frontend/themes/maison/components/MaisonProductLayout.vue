    ...
    >
      ...
      <div class="relative overflow-hidden">
        ...
        <div
          ...
          aria-hidden="true"
        >
          <span class="font-serif text-6xl text-[#8C304A]/40">{{ initial(item) }}</span>
        </div>
        ...
        <div v-if="soldOut(item)" class="absolute inset-0 flex items-center justify-center bg-[#541C2E]/45">
          <span class="rounded-full bg-[#FFFBFC]/90 px-4 py-1.5 font-sans text-[11px] font-bold tracking-wide text-[#A33D4C]">{{ t(ui.soldOut) }}</span>
        </div>
      </div>
      ...
      <div class="flex flex-1 flex-col p-5">
        ...
        <div class="mt-auto flex items-center justify-between gap-3 pt-5">
          <span class="font-serif text-xl font-bold text-[#8C304A]">
            {{ fmt(item.price) }}<span class="ml-1 text-sm font-semibold text-[#8C304A]/70">{{ ui.currency[lang] }}</span>
          </span>

          <ClientOnly>
            <template v-if="brand.ordering">
              <div v-if="order.qtyOf(item.id) > 0" class="flex items-center gap-1 rounded-full bg-[#EEDDE3] p-1">
                <button type="button" class="grid h-8 w-8 place-items-center rounded-full text-lg font-bold text-[#2C1B22] transition hover:bg-white" aria-label="−" @click="order.dec(item.id)">−</button>
                <span class="min-w-5 text-center font-serif text-sm font-bold text-[#2C1B22]">{{ order.qtyOf(item.id) }}</span>
                <button type="button" class="grid h-8 w-8 place-items-center rounded-full bg-[#8C304A] text-lg font-bold text-[#FFFBFC] transition hover:bg-[#74263E]" aria-label="+" @click="order.add(item.id)">+</button>
              </div>
              <button
                v-else-if="!soldOut(item)"
                type="button"
                class="grid h-11 w-11 place-items-center rounded-full bg-[#8C304A] text-2xl font-bold text-[#FFFBFC] shadow-[0_12px_22px_-10px_rgba(140,48,74,0.9)] transition hover:-translate-y-0.5 hover:bg-[#74263E] active:scale-95"
                aria-label="Add"
